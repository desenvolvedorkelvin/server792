...
int64_t Stats::COUNTER_DUMP_INTERVAL = 60000; // 1 min
...
void Stats::threadMain() {
    ...
    while(true) {
        ...
        if(lastCounterDump + COUNTER_DUMP_INTERVAL < OTSYS_TIME() || last_iteration) {
            writeCounter("counter.log");
            for(auto& v : tmp_counter) {
                v = 0;
            }
            lastCounterDump = OTSYS_TIME();
        }
...
void Stats::writeCounter(const std::string& file) {
        std::ofstream out(std::string("data/logs/stats/") + file, std::ofstream::out | std::ofstream::app);
    if (!out.is_open()) {
        std::clog << "Can't open " << std::string("data/logs/stats/") + file << " (check if directory exists)" << std::endl;
        return;
    ...
    int counterEnumStringsSize = sizeof(CounterEnumStrings)/sizeof(CounterEnumStrings[0]);
    
    out << std::setw(5) << "ID" << std::setw(30) << "Counter type" << std::setw(10) << "TMP COUNT" << std::setw(10) << "COUNT" << "\n";
    for(int i = 0; i < COUNTER_LAST; ++i) {
        out << std::setw(5) << i << std::setw(30) << (i < counterEnumStringsSize ? CounterEnumStrings[i] : "-") << std::setw(10) << tmp_counter[i] << std::setw(10) << counter[i] << "\n";
    }
    out << "\n";
    out.flush();
    out.close();    
